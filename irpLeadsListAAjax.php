...
--wykluczenie nowego procesu
UNION
SELECT ss.klient_id FROM `szanseSprzedazy` ss where ss.id_procesu = 62 AND (ss.status = 'otwarta' OR (ss.status = 'przegrana' AND ss.data_zakonczenia > date_sub(curdate(),interval 1 MONTH)))     
UNION 
...
UNION SELECT klient_id FROM __wykluczenia3;
...
JOIN produkty p ON t2.produkt_id = p.id 
WHERE
    k.level = 'uczestnik' 
    AND k.status = 'aktywny'
    AND NOT EXISTS(SELECT 1 FROM __wykluczenia w WHERE w.klient_id = k.id);
